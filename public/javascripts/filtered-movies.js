const moviesPerPage = 12; // Sayfa başına görüntülenecek film sayısı
let currentPage = 1;
let movies = JSON.parse(localStorage.getItem('filteredMovies'))
...
function displayPage(page) {
  ...
  if (movies.error) {
    ...
    emptyList.innerHTML = `Filtrelemeye Ait Film Bulunamadı...`;
  ...
  displayedMovies.forEach(movie => {
    ...
    overlayText.classList.add('filtered-overlay-text');
    overlayText.textContent = movie.MovieName;

    const typeText = document.createElement('div');
    typeText.classList.add('filtered-type-text');
    if (movie.TypeID === 1) {
...
function updatePagination() {
  ...
  paginationContainer.appendChild(filteredButton);
