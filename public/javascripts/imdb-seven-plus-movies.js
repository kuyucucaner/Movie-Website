...
fetch('/imdb-seven-plused-movies')
  .then(response => {
    if (!response.ok) {
...
function displayPage(page) {
  ...
  displayedMovies.forEach(movie => {
    ...
    overlayText.classList.add('imdb-overlay-text');
    overlayText.textContent = movie.MovieName;

    const typeText = document.createElement('div');
    typeText.classList.add('imdb-type-text');
...
function updatePagination(totalMovies) {
  ...
  paginationContainer.appendChild(imdbButton);
